...
async function createServer(res: http.ServerResponse): Promise<SSEServerTransport> {
  ...
  server.tool('search_notes', '搜索知识库', { query: z.string() }, async ({ query }) => {
    ...
    if (!results.length) return { content: [{ type: 'text', text: `没有找到"${query}"` }] };
    const list = results.slice(0, 10).map(n =>
  ...
  server.tool('list_notes', '列出所有笔记', {
    ...
    const text = all.slice(0, limit).map((n, i) =>
      `${i + 1}. **${n.title}** — ${n.updated_at.slice(0, 10)} (${n.id})`
    ).join('\n');
  ...
  server.tool('get_note', '获取笔记完整内容', {
    ...
    if (!note) return { content: [{ type: 'text', text: `未找到: ${id_or_title}` }] };
    const text = [
      `# ${note.title}`,
  ...
  if (url.pathname === '/mcp' && req.method === 'GET') {
    ...
    req.on('end', async () => {
      for (const transport of activeTransports.values()) {
        await (transport as any).handlePostMessage(JSON.parse(body));
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: true }));
        return;
      }
      res.writeHead(404); res.end(JSON.stringify({ error: 'No active session' }));
    });
